...
export default function Billing() {
    ...
    useEffect(() => {
        const fetchInventory = async () => {
            try {
                // We use /expiry/alerts because it returns granular batch data (price, qty, expiry)
                const res = await fetch("http://localhost:8000/expiry/alerts");
                if (res.ok) {
                    const data = await res.json();
                    // Flatten the nested structure (Drugs -> Batches) to a flat list for searching
                ...
                toast.error("Could not load inventory for billing.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            // Refresh Inventory? 
            // Ideally we re-fetch, but for now user can refresh page or we trigger generic refresh
            // We'll simplisticly verify by user refreshing.
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pt-6 h-[calc(100vh-100px)]">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold tracking-tight text-slate-900">Billing & POS</h1>
                                                ...
                                                {inventory.map((item) => (
                                                    ...
                                                    >
                                                        ...
                                                        <Check
                                                            className={cn(
                                                                "ml-2 h-4 w-4",
                                                                cart.some(c => c.batch_id === item.batch_id) ? "opacity-100" : "opacity-0"
                                                            )}
                                                        />
                                                    </CommandItem>
                                                ))}
